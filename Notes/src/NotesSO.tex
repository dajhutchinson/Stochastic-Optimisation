...
\begin{proof}{Theorem 1.3}
  ...
  Suppose none of the three inequalities holds. Then
  \[\begin{array}{rcll}
    \underbrace{\hat\mu_{1,N_1(s)}+\displaystyle\sqrt{\frac{\alpha\ln(s)}{2N_1(s)}}}_{\tiny\text{UCB($\alpha$) index 1}}&>&\mu_1&\text{contradiction to i)}\\
    &=&\mu_j+\Delta_j&\text{by def. of $\Delta_j$}\\
    &\geq&\mu_j+\displaystyle\sqrt{\frac{2\alpha\ln(s)}{N_j(s)}}&\text{by iii)}\\
    &\geq&\hat\mu_{1,N_1(s)}-\displaystyle\sqrt{\frac{\alpha\ln(s)}{2N_1(s)}}+\sqrt{\frac{2\alpha\ln(s)}{N_j(s)}}&\text{by ii)}\\
...
\begin{proof}{Theorem 1.2}
  ...
  &\leq&u+\displaystyle\int_{u-1}^\alpha2s^{-\alpha}ds\quad\text{assumption $\alpha>1$ required here}\\
